...
class Text2MotionDatasetEval(data.Dataset):
    ...
    def __getitem__(self, item):
        ...
        if m_length < self.max_motion_length:
            # 检查motion的维度
            if len(motion.shape) == 3:  # (T, 22, 3) 格式
                # 创建3维的填充：(padding_length, 22, 3)
                padding = np.zeros((self.max_motion_length - m_length, motion.shape[1], motion.shape[2]))
            elif len(motion.shape) == 2:  # (T, 66) 格式
                # 创建2维的填充：(padding_length, 66)
...
class AbsoluteMotionDataset(data.Dataset):
    def __init__(self, opt, mean, std, split_file):
        self.opt = opt
        self.data = []
        self.lengths = []
