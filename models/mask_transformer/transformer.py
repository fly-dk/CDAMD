...
class MaskTransformer(nn.Module):
    ...
    def generate(self,
        ...
        with torch.no_grad():
            cond_vector = self.encode_text(conds) #由clip完成：self.clip_model.encode_text(text).float()
        is_softmax = True
...
class ResidualTransformer(nn.Module):
    def __init__(self, code_dim, cond_mode, latent_dim=256, ff_size=1024, num_layers=8, cond_drop_prob=0.1,
        ...
        # 修改后
        # self.encode_quant = partial(F.one_hot, num_classes=self.opt.num_quantizers+1)  # +1 以包含索引 0
        ...
        self.pad_id = opt.num_tokens # 512
    ...
    def trans_forward(self, motion_codes, qids, cond, padding_mask, force_mask=False):
        ...
        # print("qids:", qids)
        # print("num_quantizers:", self.opt.num_quantizers)

        # (b, num_quantizer)

        # print("==== DEBUG qids ====")
        # print("qids:", qids)
        # print("qids.min():", qids.min().item(), "qids.max():", qids.max().item())
        # print("qids.shape:", qids.shape)
        # print("qids.dtype:", qids.dtype)
        # print("num_quantizers:", self.opt.num_quantizers)
        # assert (qids >= 0).all(), f"qids contains negative values: {qids}"
        # assert (qids < self.opt.num_quantizers).all(), f"qids out of range: {qids}, num_quantizers: {self.opt.num_quantizers}"
        # assert qids.dtype == torch.long
    ...
    def forward_with_cond_scale(self,
        ...
        # 修改后
        # q_id_value = max(1, min(q_id, self.opt.num_quantizers-1))
        # qids = torch.full((bs,), q_id_value, dtype=torch.long, device=motion_codes.device)
        if force_mask:
        ...
        # assert (qids >= 1).all() and (qids <= self.opt.num_quantizers-1).all(), f"qids out of range: {qids}"
        logits = self.output_project(logits, qids-1)
        if cond_scale == 1:
            return logits
    ...
    def generate(self,
        ...
        # print("self.pad_id:", self.pad_id) #self.pad_id: 512
        #由于motion_ids包含-1，所以需要将其clamp到0到pad_id之间
        motion_ids = motion_ids.clamp(min=0, max=self.pad_id)
