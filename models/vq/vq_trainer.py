...
class RVQTokenizerTrainer:
    ...
    def forward(self, batch_data):
        ...
        motions = motion.detach().to(self.device).float()
        motions = motions.reshape(motions.shape[0], motions.shape[1], -1)  # (B, T, 66)
        ...
        # pred_motion, kl_loss = self.vq_model(motions) # vq_model 是 VAEModel
        ...
        loss_vel = self.l2_criterion(vel_pred, vel_gt)
        pred_local_pos = pred_motion[..., 4 :]
        local_pos = motions[..., 4 :]
        loss_explicit = self.l1_criterion(pred_local_pos, local_pos)
        loss_vel = 0.1 * loss_vel + 0.1 * loss_explicit
        loss = loss_rec +  loss_vel + self.opt.commit * loss_commit
    ...
    def train(self, train_loader, val_loader, eval_val_loader, eval_wrapper, plot_eval=None):
        ...
        for epoch in tqdm(range(self.opt.max_epoch)):
            ...
            for i, batch_data in enumerate(tqdm(train_loader)):
                ...
                # no-vq
                # loss, loss_rec, loss_vel, loss_kl = self.forward(batch_data) # no-vq
            ...
            with torch.no_grad():
                for i, batch_data in enumerate(val_loader):
                    ...
                    # # for no-vq loss
                    # loss, loss_rec, loss_vel, loss_kl = self.forward(batch_data)
            ...
            # self.logger.add_scalar('Val/loss_rec', sum(val_loss_rec) / len(val_loss_rec), epoch)
            # self.logger.add_scalar('Val/loss_vel', sum(val_loss_vel) / len(val_loss_vel), epoch)
            # self.logger.add_scalar('Val/loss_kl', sum(val_loss_kl) / len(val_loss), epoch)

            # print('Validation Loss: %.5f Reconstruction: %.5f, Velocity: %.5f, KL: %.5f' %
            #       (sum(val_loss)/len(val_loss), sum(val_loss_rec)/len(val_loss), 
            #        sum(val_loss_vel)/len(val_loss), sum(val_loss_kl)/len(val_loss)))
